...
# Try to import SHAP (optional). If not installed, code will continue without SHAP.
try:
    import shap
    SHAP_AVAILABLE = True
except Exception:
...
class TravelMindPyFunc(mlflow.pyfunc.PythonModel):
    def load_context(self, context):
        ...
        # Cargar tabla base desde artifacts (fixed)
        # context.artifacts is a dict mapping artifact name to local path
...
if __name__ == "__main__":
    ...
    parser.add_argument("--generate-dummy-plots", action="store_true", help="Create dummy PNG plots and log them to MLflow (fast)")
    parser.add_argument("--tag", type=str, help="Optional tag to set on the dummy run")
    parser.add_argument("--print-model-params", action="store_true", help="Print metrics and params for a registered model version using MlflowClient")
    parser.add_argument("--model-name", type=str, default="travelmind_xgb_model", help="Registered model name to query")
    parser.add_argument("--version", type=int, default=1, help="Model version number to query")
    args = parser.parse_args()

    if args.generate_dummy_plots:
        rid = generate_dummy_plots(run_tag=args.tag)
        print("Dummy run created:", rid)
        # copy found PNGs into downloaded_artifacts for convenience
        try:
            from pathlib import Path
            out = Path(__file__).resolve().parent.parent / "downloaded_artifacts"
            out.mkdir(parents=True, exist_ok=True)
            for p in (Path.cwd() / "outputs").glob("*.png"):
                dest = out / p.name
                p.replace(dest)
            print("Copied PNGs to:", out)
        except Exception as e:
            print("Failed to copy PNGs to downloaded_artifacts:", e)
        exit(0)
    if args.print_model_params:
        ...
        try:
            ...
            df_metrics = pd.DataFrame(metrics.items(), columns=["Métrica", "Valor"]) if metrics else pd.DataFrame()
            df_params = pd.DataFrame(params.items(), columns=["Parámetro", "Valor"]) if params else pd.DataFrame()
            print(df_metrics)
            print(df_params)
        except Exception as e:
            print(f"Failed to fetch model version info: {e}")
        exit(0)
