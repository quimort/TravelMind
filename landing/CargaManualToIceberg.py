...
import utils  # tu módulo con create_context y append_iceberg_table
...
# --- CONFIGURACIÓN ---
API_KEY = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJmcnZhcmdhcy44N0BnbWFpbC5jb20iLCJqdGkiOiI3MTJmNjFkYi1hMDg3LTRkM2QtODFlNS04ZjY4YjYwOWE2YTAiLCJpc3MiOiJBRU1FVCIsImlhdCI6MTc0OTIyOTY1OSwidXNlcklkIjoiNzEyZjYxZGItYTA4Ny00ZDNkLTgxZTUtOGY2OGI2MDlhNmEwIiwicm9sZSI6IiJ9.BbMqB0Jj2_z5wJw6luQhH7iMlJDMk2gfPEVOQ7Chc7E"   # pon tu clave de AEMET
JSON_PRINCIPAL = "aemet_unificado_2010-01-01_2024-12-31.json"
DB_NAME = "landing_db"
TABLE_NAME = "aemetClimaDiario"

# --- Esquema de la tabla (el mismo que usabas antes) ---
esquema = StructType([
    ...
    StructField("presMax", StringType()),
    StructField("horaPresMax", StringType()),
    StructField("presMin", StringType()),
...
if __name__ == "__main__":
    ...
    print("Warehouse:", spark.conf.get("spark.sql.catalog.spark_catalog.warehouse"))

    # 2. Cargar JSON principal si existe
    if os.path.exists(JSON_PRINCIPAL):
        print(f"\n📥 Cargando datos del archivo principal {JSON_PRINCIPAL} en Iceberg...")
        cargar_json_a_iceberg(
            spark,
            JSON_PRINCIPAL,
            esquema,
            db_name=DB_NAME,
        ...
        print(f"❌ No se encontró {JSON_PRINCIPAL}, omitiendo carga inicial.")

    # 3. Reintentar errores y cargar sus JSON
    print("\n🔄 Reintentando intervalos fallidos del log...")
    ...
    if archivos_error_generados:
        for archivo in archivos_error_generados:
            if os.path.exists(archivo):
                print(f"\n📥 Cargando datos reintentados desde {archivo} en Iceberg...")
                cargar_json_a_iceberg(
                    spark,
                    archivo,
                    esquema,
        ...
        print("✅ No había errores pendientes o ya se resolvieron.")
