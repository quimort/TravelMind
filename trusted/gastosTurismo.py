...
# 3) Provinces of interest (by destino)
provinces = ["Madrid", "Barcelona", "Sevilla", "Illes Balears", "Valencia"]

print("\n=== Province Data Quality Report (gastos_Turismo) ===")

for prov in provinces:
    df_prov = df.filter(col("PROVINCIA_DESTINO") == prov)
    count_total = df_prov.count()

    df_prov_clean = df_clean.filter(col("PROVINCIA_DESTINO") == prov)
